...
exports.stkPush = async (req, res) => {
  try {
    ...
    await Payment.create({ user: user._id, phone, amount, type: "deposit", reference, status: "pending" });
    await Transaction.create({ user: user._id, amount, type: "deposit", status: "pending", reference, phone });
    ...
    res.json({ message: "STK sent. Check your phone.", reference });
...
exports.payheroCallback = async (req, res) => {
  try {
    ...
    if (code === 0 || code === "0") {
      ...
      console.log(`✅ Deposit: ${ref} — KES ${payment.amount} → ${user.phone}`);
    } else {
      payment.status = "failed";
      await payment.save();
      if (tx) { tx.status = "failed"; await tx.save(); }
      console.log(`❌ Deposit failed: ${ref}`);
    }
